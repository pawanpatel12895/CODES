...
class Tree
{
        ...
        int maxSum(Node *t)
        {       if(t==NULL)return 0;
                return t->data+max(maxSum(t->left),maxSum(t->right));
        }
        int printPath(Node *t,int &m,int sum)
        {       if(t==NULL&&m==sum)return 1;
                if(t==NULL) return 0;
                if(printPath(t->left,m,t->data+sum)==1||printPath(t->right,m,t->data+sum)==1)
                {        cout<<t->data<<' ';
                        return 1;
                }
                return 0;
