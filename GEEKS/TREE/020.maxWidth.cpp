...
class Tree
{
        ...
        int maxWidth(Node *t,vector<int> &count,int lev)
        {       if(t==NULL)return 0;
                count[lev]++;
                
                 int a=maxWidth(t->left ,count,lev+1);
                 int b=maxWidth(t->right,count,lev+1);
                 return max(count[lev],max(a,b));
        }
       public:
...
{       int A[]={1,34,6,4,2,4,7,5,4,5,4,3,22,2};
