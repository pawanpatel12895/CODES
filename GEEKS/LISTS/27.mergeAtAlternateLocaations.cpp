...
{       struct Node
        ...
        Node *mergeAlternate(Node *rt,Node *root,int k)
        {       if(rt==NULL)return root;
                if(root==NULL)return rt;
                Node *t;
                if(k%2)
                {       t=rt;
                        t->next=mergeAlternate(rt->next,root,k+1);
                }
                else
                {       t=root;
                        t->next=mergeAlternate(rt,root->next,k+1);                
                }
                return t;
        }
       public:
        ...
        void mergeAlternate(List *M)
        {       M->mergeAlternate(root);
                root=NULL;
        }
        void mergeAlternate(Node *rt)
        {       root=mergeAlternate(rt,root,0);   
                
        }
};

int main(int argc, char *argv[])
{       int A[]={1,2,3,4,5,5,6,77,8,8};
        int B[]={7,5,4,4,3,2,2};
