...
{       struct Node
        ...
        {       if(rt==NULL)return NULL;
                rt->next=abcd(rt->next);
                int c;
                if(rt->next==NULL)c=-INF;
                else c=rt->next->data;
                if(rt->data < c)       
                {       Node *t=rt->next;
                        free(rt);
       ...
       public:
        ...
        void MergeSort()
        {       root=MergeSort(root);   }        
        void RemoveElementsWhichHaveGreaterElementsInRightSide()
        {       root=abcd(root);
...
{       int A[]={34,5,2,4,25,23,53,52,2};
        int n=sizeof(A)/sizeof(A[0]);
        List L;
        while(n--)L.push_front(A[n]);
        L.display();
        L.RemoveElementsWhichHaveGreaterElementsInRightSide();
        L.display();
