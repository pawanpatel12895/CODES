...
class Tree
{
        ...
        int pathSum(Node *t, int sum,int S)
        {       if(t->left==NULL&&t->right==NULL)
                {       if(sum==S+t->data)return 1;
                        else return 0;
                }
                if(t->left)
                        if(pathSum(t->left,sum,S+t->data)==1)return 1;
                if(t->right)
                        if(pathSum(t->right,sum,S+t->data)==1)return 1;
                return 0;
        }
       public:
        ...
        int pathSum(int sum)
        {       if(root==NULL)if(sum==0)return 1;
                              else return 0;
...
{       Tree T;
        int A[]={1,5,2,6,9,4,2,56,8,2};
        int n=sizeof(A)/sizeof(A[0]);
        while(n--)
                T.insert2(A[n]);
        T.inorder();
        for(int i=0;i<100;i++)
        cout<<i<<' '<<T.pathSum(i)<<endl;
