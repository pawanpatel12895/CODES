...
class Tree
{
        ...
        void verticalSum(Node *,map<int,int> &S,int);
       ...
       public:
        ...
        void verticalSum()
        {       map<int,int> S;
                verticalSum(root,S,0);      
                print(S);
        }
};
void Tree::print(map<int,int> &M)
{
...
void Tree::verticalSum(Node *t,map<int,int> &S,int i)
{       if(t==NULL)return;
        verticalSum(t->left,S,i-1); 
        S[i]+=t->data;
        verticalSum(t->right,S,i+1);
}

int main(int argc, char *argv[])
{       int A[]={1,2,3,4,6,7,8,8,4,2,3,6,5};
