...
class Tree
{
       ...
       public:
        ...
        void setByLevel();
        void displayByLevel(){displayByLevel(root);}
};
void Tree::setByLevel()
{       deque<pair<Node*,int> > D;
        ...
        while(!D.empty())
        {
                ...
                D.pop_front();
                if(P.first->left)
                        D.push_back(make_pair(P.first->left,P.second+1));
                if(P.first->right)
                        D.push_back(make_pair(P.first->right,P.second+1));
                if(D.empty()||D.front().second!=P.second)
                        P.first->next=NULL;
                else    P.first->next=D.front().first;
        }
}
void Tree::displayByLevel(Node *t)
{     if(t==NULL)return;  
      ...
      if(t->next)
        cout<<t->next->data<<' ';
      cout<<endl;
      displayByLevel(t->left);
      displayByLevel(t->right);
}
