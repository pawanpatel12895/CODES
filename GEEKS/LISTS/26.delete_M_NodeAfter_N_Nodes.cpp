...
{       struct Node
        ...
        Node *deleteMafterN(Node *root,int M,int N)
        {       if(root==NULL)return NULL;
                if(M)root->next=deleteMafterN(root->next,M-1,N);
                else if(N)
                {        root->next=deleteMafterN(root->next,M,N-1);
                        Node *t=root->next;
                        free(root);
                        return t;
                }
       ...
       public:
        ...
        {       root=deleteMafterN(root,M,N);       }

};

int main(int argc, char *argv[])
{
        int A[]={1,2,3,4,5,6,6,7,78,8,8,8,7,6,};
